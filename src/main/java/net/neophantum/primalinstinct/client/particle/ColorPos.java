...
/**
 * Stores a 3D position and a particle color, for use in rendering effects like HUD indicators.
 */
public class ColorPos {
    ...
    public static ColorPos centeredAbove(BlockPos pos, ParticleColor color) {
        return new ColorPos(new Vec3(pos.getX() + 0.5, pos.getY() + 1.0, pos.getZ() + 0.5), color);
    }

    public static ColorPos centeredAbove(BlockPos pos) {
        return new ColorPos(new Vec3(pos.getX() + 0.5, pos.getY() + 1.0, pos.getZ() + 0.5));
    }
    ...
    public static ColorPos fromTag(CompoundTag tag) {
        return new ColorPos(
                ...
                ParticleColorRegistry.from(tag.getCompound("color"))
        );
    }
}
